...
def impose(project_dir, project, booklet = False):
    ...
    """
    reader = PdfReader(PDF_PATH_FMT.format(project_dir=project_dir, project=project))
    writer = PdfWriter()
    if booklet:
        dest = PDF_BOOKLET_PATH_FMT.format(project_dir=project_dir, project=project)
    else:
...
def main():
    ...
    print("Imposing PDFs...")
    impose(project_dir, args.project, booklet=False)
    impose(project_dir, args.project, booklet=True)
